...
namespace Tests
{
  ...
  void TestRational()
  {
  ...
  #ifdef VG
    //Kollar att int r臾nar med 64 bitar internt
    bool TestAccuracy() {
      Rational<int> left(numeric_limits<int>::max(), 2), right(left), result;
      result = left + right;
      assert(result == Rational<int>(numeric_limits<int>::max(), 1));
      return true;
    }
    ...
    // Testar om Rational 舐 komptaibelt med int, shar, long long
    //  anv舅der + f att prova detta.
  ...
  void TestRationalG() {
    ...
    //Konstrueras fr蚣 乃al・dvs. Rtal rtal(tal);
    RLL rllx(1);
    RLL rlly(rs0);

    //J舂fas med == dvs. if (rtal == tal) ・
    assert(rs1 == rs1);
    assert(rs2 == 2);
    assert(rs1 == rll1);
    ...
    ////un舐t 薄・dvs. rtal1 = -rtal2;
    assert((rs0 = -rs1) == Rshort(-1));

    //b蘚a ++ operatorerna, dvs. ++rtal; rtal++;
    rll3 = RLL(1, 6);
    assert(++rll3 == RLL(7, 6));
    assert(rll3++ == RLL(7, 6));
    assert(rll3 == RLL(13, 6));

    //// explicit konvertering till Tal. (Kr舸er VS2012 och kompilator CTnom november 12.
    int i = (int)static_cast<Rational<short>>(rll3);
    assert(i == 2);
